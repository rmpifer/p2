...
<script>
	$("#createUser").submit(function(event) {
		...
		if (!re.match("^[\w\d_]*$", checkUsername)){
		...
		if (message.length == 0){
			...
			$.ajax({
				...
				success: function(data) { window.location.replace("http://0.0.0.0/8000/login")},
